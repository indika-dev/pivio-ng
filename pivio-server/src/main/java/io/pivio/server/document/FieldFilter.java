...
class FieldFilter {
  ...
  JsonNode filterFields(JsonNode document, List<String> fields) {
    ...
    while (allFields.hasNext()) {
      ...
      if (fieldsWithSubFields.contains(currentField.getKey())) {
        ...
        JsonNode subNode =
            filterFields(subDocument, getSubFieldForField(currentField.getKey(), fields));
  ...
  private ObjectNode createSubDocumentWithItsChildrenAsTopLevelAttributes(
      Map.Entry<String, JsonNode> currentField) {

...
public class DocumentController {
  ...
  public ResponseEntity<String> create(@RequestBody ObjectNode document,
    ...
    switch (indexResponse.result()) {
      ...
      case Created:
      case Updated:
        logger.info("indexed {} successfully with result: {}", document.get("id"),
  ...
  private JsonNode missingMandatoryField(JsonNode document) {
    ...
    if (missingMandatoryField != null) {
      logger.info("Received document with missing mandatory field in {}", document.toString());
      error.put("error", "mandatory field '" + missingMandatoryField + "' is missing");
    } else {
      logger.info("Received document with empty mandatory field in {}", document.toString());
  ...
  private JsonNode missingIdError(JsonNode document) {
    logger.info("Received document without or with empty id field in {}", document.toString());
  ...
  public ResponseEntity<Void> delete(@PathVariable String id) throws IOException {
    logger.info("Try to delete document {}", id);
    ...
    switch (response.result()) {
      case Deleted:
        ...
        if (changesetResponse.result() == Result.Deleted) {
          ...
          logger.info("Deleted document {} successfully", id);
          return ResponseEntity.noContent().build();
        } else {
          logger.warn("Could not delete document {}", id);
          return ResponseEntity.notFound().build();
        }
